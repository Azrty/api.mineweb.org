...
router.post('/upload', upload.single('file'), function (req, res, next) {
    ...
    JSZip.loadAsync(req.file.buffer).then(function (zip) {
        ...
        zipFolder.file('config.json').async('string').then(function (config) {
            ...
            var model = (type === 'THEME') ? Theme : Plugin;
            var data = (type === 'THEME') ? {supported: config.supported} : {requirements: config.requirements};
            model.update({id: id}, data).exec(function (err) {
                if (err) {
                    console.error(err);
